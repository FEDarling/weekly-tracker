...
## [](https://simonplend.com/guidelines-for-choosing-a-node-js-framework/#conclusion)
## 你的具体使用情况如何?
​
...
## 项目健康
在选择用于构建项目的框架时，你肯定希望能够保证项目能正常运行，并且在可预见的未来将继续得到维护。以下是一些需要寻找的指标：

- **Releases**。一些开发人员认为，如果框架已经“功能完整”，那它不需要新版本了，但安全更新和错误修复是必要的。框架（或其依赖项之一）中始终存在重大漏洞的可能性。避免任何未维护且不再发布版本的框架是明智的。
- **issues积极度**。维护人员最近没有回应的许多问题可能表明该项目未得到维护。另一方面，如果根本没有问题，则可能表明该框架的使用率非常低。
- 拉取请求。一个健康的项目通常会有一些最近的拉取请求活动。许多旧的和不活动的开放拉取请求可能表明该项目不再被维护。
- **贡献者**。如果一个框架只有一两个贡献者，则可能表明它周围没有太多社区。如果这些贡献者退出项目，它也会对该框架的长期维护产生风险。一个健康的项目将有许多贡献者，添加大大小小的变化。
