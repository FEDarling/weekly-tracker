...
这正是 V8 使用的模型。 V8 的解释器称为 **引燃(Ignition)**，它是所有引擎中最快的解释器（就原始字节码执行速度而言）。 V8 的优化编译器名为 **涡轮风扇(TurboFan)**，它最终会生成高度优化的机器代码：
...
V8 开始在 **引燃(Ignition)** 解释器中运行字节码。在某个时刻，引擎确定代码达到了 hot 的程度，并启动 **涡轮风扇(TurboFan)**，这是 **涡轮风扇(TurboFan)** 处理集成分析数据和构建代码的基本机器表示的部分。然后将其发送到不同线程上的 **涡轮风扇(TurboFan)** 优化器以进行进一步改进：

![](https://cdn.jsdelivr.net/gh/daodaolee/photobed@main/img/20220114005434.png)

当优化器运行时，V8 继续在 **引燃(Ignition)** 中执行字节码。在某些时候优化器完成了，我们有了可执行的机器代码，并且可以继续执行。
